...
After ingestion, the pipeline removes incomplete rows and leap-day observations, calculates historical distributions for each calendar day, and derives the current-year ranges, records, extremes, and annual trend used by the dashboard. The most recent date is omitted from the current-year display until the Berlin local calendar day has ended and it contains the expected number of hourly observations, including across daylight-saving transitions.
