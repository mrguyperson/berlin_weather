...
::: {.valuebox icon="activity" color="light"}
**Latest-day anomaly:**

`r glue("{if (nrow(latest_day_summary) == 0) '—' else sprintf('%+.1f\u00A0°C', latest_day_summary$anomaly)}")`
...
```
Historical bands and mean pool hourly temperatures observed on each calendar date across prior years; black marks show each completed day's minimum–maximum temperature this year.
...
**Most extreme temperatures on completed days**
``` {r}
#| label: hottest_temps
